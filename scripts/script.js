$(document).ready(function() {
  ...
  function saveText(event) {
    ...
    if (target.tagName == "DIV") {
      ...
      rowElement = target.parentElement.parentElement.getAttribute("data-index");
      keyName = "hour-" + rowElement;
    }
  ...
  if (isAM) {
    ...
    if (currentHour == 12) {
      $(".hour[data-hour=12]").next().addClass("present");
      $(".hour[data-hour=1]").next().addClass("future");
      $(".hour[data-hour=2]").next().addClass("future");
      $(".hour[data-hour=3]").next().addClass("future");
      $(".hour[data-hour=4]").next().addClass("future");
      $(".hour[data-hour=5]").next().addClass("future");

    } else {
      $(".hour[data-hour=12]").next().addClass("past");
